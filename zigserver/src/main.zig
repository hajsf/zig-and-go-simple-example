...
fn handleRequest(response: *http.Server.Response, allocator: std.mem.Allocator) !void {
    ...
    if (std.mem.startsWith(u8, response.request.target, "/")) {
        ...
        std.log.info("trying to open the file", .{});
        const contents = try readFile(allocator, "index.html");
        defer allocator.free(contents);
        std.log.info("File had been read", .{});
        //    std.log.info("{s}", .{str});
        //_ = c;
        ...
        // Set "content-type" header to "text/plain" or text/html.
        try response.headers.append("content-type", "text/html"); // text/plain
